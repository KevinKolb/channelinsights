"""
Generate pos_providers.json with North American POS system providers.
Data Source: Wikidata (https://www.wikidata.org/) - CC0 1.0
Usage: python scripts/pos_providers.py
"""
import json
import os
...
def fetch_pos_providers_from_wikidata() -> Optional[List[Dict]]:
    """Fetch POS (Point of Sale) system providers."""
    endpoint = "https://query.wikidata.org/sparql"
    query = """
    SELECT DISTINCT ?entity ?entityLabel ?countryLabel ?websiteUrl WHERE {
      {
        ?entity wdt:P452 wd:Q7091182.
      } UNION {
        ?entity wdt:P1056 wd:Q1172284.
      }
    ...
    try:
        print("Fetching POS providers from Wikidata...")
        ...
        if not results:
            print("Warning: No POS providers found")
            return None
        ...
        print(f"Found {len(entities)} POS providers")
        return entities

    except requests.RequestException as e:
        print(f"Error fetching POS providers from Wikidata: {e}")
        return None
...
def generate_pos_providers_data() -> Dict:
    """Generate POS providers data with automatic fallback to hardcoded data."""
    entities = fetch_pos_providers_from_wikidata()

    if not entities:
        print("Using hardcoded fallback data...")
        entities = get_hardcoded_pos_providers()
...
def main():
    """Generate and save pos_providers.json to data directory."""
    print("Generating North American POS providers data...")
    data = generate_pos_providers_data()

    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_dir = os.path.join(os.path.dirname(script_dir), 'data')
    os.makedirs(data_dir, exist_ok=True)

    output_file = os.path.join(data_dir, 'pos_providers.json')
